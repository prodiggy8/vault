...
- CIDR: flexible division (slash prefix)
- **Aggregates** forwarding table entries “in the same general direction”
...
ISPs advertise blocks of addresses to neighboring ISPs

Routers use **longest prefix matching algorithm**
- Packet may match multiple table entries
- Send to the one with longest prefix
- Longest prefix: closer to host

**How to populate forwarding tables?**
- LANs use broadcasting, learning, spanning tree
- Internet uses Border Gateway Protocol

### Transport Layer

- Process multiplexing via ports
- **TCP:** *in-order*, two-way byte stream service
	- Uses ACKs and sequence numbers to do in-order
- Error control: recover from packet loss, corruption, reordering
	- Retransmit after timeout (estimated based on round trip time)
- Congestion control: controls the transmit rate of the sender

**Sliding window:** instead of waiting for ACK to send another packet, uses a window (sends $n$ at time).
- Selectively re-sends lost packets (after timeout, sliding window keeps going in the meantime)
- Cannot advance beyond earliest un-ACKed package (i.e., lost package must timeout otherwise it consumes space in the window)

**Congestion Control:**
- Slow start (exponential) to discover bandwidth
- After discovery, starts probing linearly for more bandwidth
- After loss, it halves the transmission rate

#### Takeaways
- Compare flat and hierarchical addressing
- Discuss design decisions in class-based vs. classless IP addressing
- Explain advantages of route aggregation
- Explain TCP mechanisms