...
I have read the course and university policies on academic integrity and I agree to abide by them through the semester and beyond.
...
Each iteration costs $O(p)$. $O(p)$ for `DeterministicSelect` and $p-1$ extra for `Partition`. The values of $p$ are $n, 3^i, 3^{i-1}, \dots, 1$, where $3^i \leq n$, so the total is: $$O\left( n + 3^i + \dots + 1 \right) = O\left( n + \frac{3}{2} \cdot 3^i \right)=O(n)$$
1. c) **Almost Sorting**
...
By part a, $D$ and $E$ together contain exactly the elements of $A$ and are sorted. Merging (from MergeSort) given two sorted arrays produces a sorted array with the same elements from both inputs (as per previous courses).
...
An algorithm making at most $c$ comparisons is a decision tree of depth $\leq c$, with at most $2^c$ leaves and hence at most $2^c$ distinct outputs. So $2^c ≥ 2^{n−1}$, giving $c \geq n − 1$. This matches the $O(n)$ algorithm of part (b), which is therefore asymptotically optimal.
...
Feed the algorithm the array $1, 2, \dots, n$. Suppose it finishes in fewer than $n-3$ comparisons. Then there’s some neighboring pair of positions $i$ and $i+1$ (with $2 \leq i \leq n-2$ so both streaks have length $\geq 2$) that it never compared directly.
...
However, since the second array is a valid input (with streaks $(1, \dots, i-1,i+1$) and $(i, i+2, \dots, n)$) and the two arrays are different orderings of the same numbers, they need different rearrangements to sort them. Hence, the algorithm must make at least $n-3 \in \Omega(n)$ comparisons.
...
Edge case: if $t$ doesn’t divide $n$, then use $\left\lfloor  \frac{n}{t}  \right \rfloor$ columns and tack the leftover values onto the end in increasing order. For $t=1$ with $n < 4$, the bound is trivial.
